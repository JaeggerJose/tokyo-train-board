...
### ワンコマンド導入（推奨。csl 不要・JSON 編集不要）

```bash
pip install tokyo-train-board       # または pipx install tokyo-train-board
jrboard install-statusline          # statusLine を ~/.claude/settings.json に書き込む（自動バックアップ・他キー保持）
```

> `install-statusline` はコマンドを `<あなたの python> -m jrboard` として生成するので、**`jrboard` が PATH 上になくても動きます**（`pip install --user` は `~/.local/bin` に置くため）。`--columns 90`／`--line oedo`／`--city Tokyo`／`--mode minitable` でカスタマイズ可。削除は `jrboard --uninstall-statusline`。

**最近の Debian/Ubuntu（PEP 668 が `pip install` をブロック）** は venv（sudo 不要）：
```bash
python3 -m venv ~/.jrboard && ~/.jrboard/bin/pip install tokyo-train-board
~/.jrboard/bin/python -m jrboard install-statusline   # コマンドはこの venv の python を自動参照
```
（`python3 -m venv` すら無い場合は `sudo apt install python3-venv`、または `pipx`。）

### 手動設定（コマンドをカスタマイズする場合）
...
**レスポンシブ幅（RWD）**：内容は `--columns`（またはテーマの `JR_COLUMNS`）の幅予算に合わせて再配置され、**決してはみ出しません**。広い行ではフルのマーキー + `5h · 7d · ctx` ゲージを表示し、狭くなるとゲージは順に削減（まず `ctx`、次に `7d`、`5h` を最後まで残す）、さらに狭いとゲージを完全に省略して電車のみにします。`JR_COLUMNS` を端末幅に設定してください（statusLine は実際の幅を取得できないため、この数値が予算になります）。
...
- **`--by-project`** — 安定ハッシュで Claude のプロジェクトディレクトリ（`workspace.project_dir`、無ければ `cwd`）から路線を決定：**同じプロジェクトのセッションは常に同じ路線、新しいプロジェクトごとに専用の路線**になります。`--claude-stdin` が必要で、`--city` で範囲を絞れ、`--by-session` より優先。注意：路線を追加するとプールが変わり、一部のプロジェクトの路線が変わることがあります。
...
- **`--rotate [MIN]`**（statusline / minitable）— （`--city` で絞った）路線プールを時間バケットでローテーション。`MIN` 分ごとに進む（値なし＝0.5 分＝30 秒）。同一バケット内は同じ路線、バケットをまたぐと切り替わります。`--by-project` / `--by-session` が `--rotate` より優先。
...
### プロジェクト / セッションごとにステータスラインを変える方法

1. **`JR_BY_PROJECT=1`（`jr-board` / `jr-timetable` / `jr-status` テーマの既定）**：Claude プロジェクトごとに路線を固定し、プロジェクトを切り替えると路線も変わります。`0` で無効化。
2. **`JR_BY_SESSION=1`**：代わりに `session_id` で変化させ、異なる Claude セッションは異なる路線を表示（by-project が優先されるため `JR_BY_PROJECT=0` も設定）。
3. **プロジェクト単位の `.claude/settings.json` 上書き**：あるリポジトリにそのプロジェクト専用の `statusLine.command` を置く（例：`--line oedo --station tochomae` で固定、または別の `--city`）と、そのプロジェクト専用のステータスラインになります。
...
どちらも既定で `JR_BY_PROJECT=1` と `JR_TOKENS=1`。各 `.sh` 冒頭の `JR_CITY` / `JR_ROTATE` / `JR_LINE` / `JR_STATION` / `JR_COLUMNS` で調整できます。
